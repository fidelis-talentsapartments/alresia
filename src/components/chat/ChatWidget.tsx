...
import { MessageCircle, X, Send, Sparkles, Headphones, History, Plus } from "lucide-react";
...
export function ChatWidget() {
  ...
  const onMessagesChange = useCallback(
    (msgs: Parameters<typeof updateMessages>[1]) => {
      if (activeId) updateMessages(activeId, msgs);
    },
    [activeId, updateMessages]
  );

  const { messages, isLoading, error, sendMessage, clearMessages, resetMessages } =
    useAiChat({
  ...
  useEffect(() => {
    resetMessages(activeConversation?.messages ?? []);
  }, [activeId]);
  ...
  const handleSend = (text?: string) => {
    ...
    if (!activeId) {
      ...
      // We need to wait for the state to update, so we set it up
      // The onMessagesChange will use the new activeId after re-render
      setTimeout(() => {
        // activeId will be newId after re-render
      }, 0);
    }
    if (!text) setInput("");
    sendMessage(trimmed);
              ...
              <div className="bg-primary px-5 py-4 flex items-center justify-between flex-shrink-0">
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    className="h-8 w-8 text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/10 relative"
                    title="Conversation history"
                  >
                    <History className="w-3.5 h-3.5" />
                    {conversations.length > 0 && (
                      <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-chart-1 text-[9px] font-bold flex items-center justify-center text-primary-foreground">
